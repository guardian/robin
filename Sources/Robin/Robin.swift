...
public class Robin: NSObject, ObservableObject {
    ...
    var signposter = OSSignposter(subsystem: "Robin", category: .pointsOfInterest)
    ...
    deinit {
        signposter.emitEvent("Deinit", "\(self.currentMedia?.metadata?.title ?? "no info")")
        ...
        timeObserverToken.map { player.removeTimeObserver($0) }
        timeObserverToken = nil
        timeControlStatusObserver?.invalidate()
        timeControlStatusObserver = nil
        bufferingStatusObserver?.invalidate()
        bufferingStatusObserver = nil
        NotificationCenter.default.removeObserver(self, name: .AVPlayerItemDidPlayToEndTime, object: nil)
    }

    public func removePlayer() {
        signposter.emitEvent("removePlayer", "\(self.currentMedia?.metadata?.title ?? "no info")")
        do {
            signposter.emitEvent("AVAudioSession is inactive", "\(self.currentMedia?.metadata?.title ?? "no info")")
    ...
    private func preparePlayer() {
        do {
            ...
            do {
                signposter.emitEvent("AVAudioSession is active", "\(self.currentMedia?.metadata?.title ?? "no info")")
    ...
    func addInterruptionNotificationObserver() {
        signposter.emitEvent("observer added", "\(self.currentMedia?.metadata?.title ?? "no info")")
    ...
    func removeInterruptionNotificationObserver() {
        signposter.emitEvent("observer removed", "\(self.currentMedia?.metadata?.title ?? "no info")")
...
extension Robin {
    ...
    private func observeTimeChanges() {
        print("[!] observeTimeChanges: \(player)")
        if let token = timeObserverToken {
            player.removeTimeObserver(token)
            timeObserverToken = nil
        }

        timeObserverToken = self.player.addPeriodicTimeObserver(forInterval: CMTime(value: 1, timescale: 1000),
    ...
    private func observeCurrentState() {
        print("[!] observeCurrentState: \(player)")
        ...
        timeControlStatusObserver = player.observe(\.timeControlStatus) { [weak self] _, change in
            print("[!] timeControlStatus: \(self?.player)")
            switch self?.player.timeControlStatus {
            case .paused:
                guard self?.player.currentItem?.status == .readyToPlay else { return }
                if self?.audioCompleted() ?? false {
                    self?.signposter.emitEvent("audio finished", "\(self?.currentMedia?.metadata?.title ?? "no info")")

                    self?.audioObserverStateChanged(state: .finished)
//                    self?.replay()
                } else {
                    self?.signposter.emitEvent("audio paused", "\(self?.currentMedia?.metadata?.title ?? "no info")")
                    self?.audioObserverStateChanged(state: .paused)
                }
            case .waitingToPlayAtSpecifiedRate:
                self?.audioObserverStateChanged(state: .buffering)
            case .playing:
                self?.signposter.emitEvent("audio playing", "\(self?.currentMedia?.metadata?.title ?? "no info")")
...
extension Robin {
    ...
    public func play() {
        guard player != nil else {
            print("[!] play called with no player - ignoring")
            signposter.emitEvent("play, no player", "\(self.currentMedia?.metadata?.title ?? "no info")")
            return
        }
        signposter.emitEvent("play", "\(self.currentMedia?.metadata?.title ?? "no info")")
//        preparePlayer()
        print("[!] play: \(player)")
    ...
    public func pause(removeFromNowPlaying: Bool = false) {
        signposter.emitEvent("pause", "\(self.currentMedia?.metadata?.title ?? "no info")")
    ...
    public func changePlaybackRate(rate: Float) {
        print("[!] changePlaybackRate: \(player)")
        player.rate = rate
...
extension Robin {
    ...
    private func setupNowPlaying(sound: RobinAudioSource) {
        ...
        nowPlayingInfo[MPNowPlayingInfoPropertyElapsedPlaybackTime] = CMTimeGetSeconds(player.currentTime() ?? CMTime(seconds: 0, preferredTimescale: CMTimeScale()))
    ...
    private func updateNowPlaying() {
        DispatchQueue.main.asyncAfter(deadline: .now()) {
            ...
            nowPlayingInfo[MPNowPlayingInfoPropertyElapsedPlaybackTime] = CMTimeGetSeconds(self.player.currentTime() ?? CMTime(seconds: 0, preferredTimescale: CMTimeScale()))
...
extension Robin: AVAudioPlayerDelegate {
    ...
    private func playerDidFinishPlaying() {
        signposter.emitEvent("didFinish", "\(self.currentMedia?.metadata?.title ?? "no info")")
        MPNowPlayingInfoCenter.default().nowPlayingInfo = nil
